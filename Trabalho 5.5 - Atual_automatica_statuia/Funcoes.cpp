...
void menu_secundario(int quantidade_filmes,Filmes retorno[MAX])  // MENU VISULIZAÇÕES
{
        ...
        case 1:  // VIAZUALIXZAR TODOS OS FILMES
            ...
            cout << "quantidade filmes: " <<quantidade_filmes;
...
{
    cout << "quantidade filmes: " <<quant_filmes;
    Data data_atual;
    ...
    {
        if ((data_atual.dia>= inf[i].data.dia) && (data_atual.mes>=inf[i].data.mes) && (data_atual.ano>=inf[i].data.ano) && (inf[i].status == 4))
        {
            inf[i].status=5;
        }
